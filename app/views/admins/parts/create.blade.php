...
@section('content')
  <div class="page-header clearfix">
    <h3><span class="glyphicon glyphicon-folder-close"></span>
      <span class="glyphicon-class">NEW PART</span>
    </h3>
  </div>
  <div class="row">
    <form enctype="multipart/form-data" role="form" method="POST" action="{{url("admin/part/create")}}">
      {{Form::token()}}
      <div class="col-xs-12">
        <div class="form-group">
          <label for="Number">Part number</label>
          <input type="text" name="number" class="form-control" id="number" placeholder="Enter part number here" value="{{Input::get('number', '')}}">
        </div>
        <div class="form-group">
          <label for="name">Name</label>
          <input type="text" name="name" class="form-control" id="name" placeholder="Enter part name here" value="{{Input::get('name', '')}}">
        </div>
        <div class="form-group">
          ...
          <select class="chosen-select form-control" name="product_id" id="product_id" data-placeholder="Choose a product">
            @foreach ($products as $product)
              @if ($product->id == Input::old('product_id'))

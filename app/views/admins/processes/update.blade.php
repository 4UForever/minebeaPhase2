...
@section('content')   
  <div class="page-header clearfix">
    <h3><span class="glyphicon glyphicon-wrench"></span>
      <span class="glyphicon-class">EDIT PROCESS: <i>{{$process->title}}</i></span>
    </h3>
  </div>
  <div class="row">                                                                   
    <form role="form" method="POST" action="{{url("admin/process/{$process->id}/update")}}">
      {{Form::token()}}
      <div class="col-xs-12">                                                                               
        <div class="form-group">              
          <label for="title">Title</label>
          <input type="text" name="title" class="form-control" id="title" value="{{$process->title}}">
        </div>                                                                                      
        <div class="form-group">              
          <label for="number">Number</label>
          <input type="text" name="number" class="form-control" id="number" value="{{$process->number}}">
        </div>    
        <div class="form-group">              
          <label for="group">Line</label>   
          <select class="chosen-select form-control" name="line_id" data-placeholder="Choose a line">    
            @foreach ($lines as $line)                
              @if ($line->id == $process->line->id)         

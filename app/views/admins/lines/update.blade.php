...
@section('content')   
  <div class="page-header clearfix">
    <h3><span class="glyphicon glyphicon-th"></span>
      <span class="glyphicon-class">EDIT PRODUCTION LINE: <i>{{$line->title}}</i></span>
    </h3>
  </div>
  <div class="row">                                                                   
    <form role="form" method="POST" action="{{url("admin/line/{$line->id}/update")}}">
      {{Form::token()}}
      <div class="col-xs-12">                                                                           
        <div class="form-group">              
          <label for="title">Title</label>
          <input type="text" name="title" class="form-control" id="title" value="{{$line->title}}">
        </div>      
        <div class="form-group">              
          ...
          <select class="chosen-select form-control" multiple="multiple" name="product_ids[]" data-placeholder="Choose at least one model or create a new one">    
            @foreach ($products as $product)
              @if (in_array($product->id, $line_product_ids))        
        ...
        <div class="form-group">                              
          <div class="page-header clearfix">          
            <a href="{{url('admin/process/create-form')}}" id="add-new-process" class="btn btn-info">
              <span class="glyphicon glyphicon-plus-icon"></span>
              <span class="glyphicon-class">Processes</span>
            </a>  
          </div>
          {{$new_processes_table}}  
        </div>                                                                                                                              
